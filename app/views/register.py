...
def register_management(request):
    ...
    for r in content:
        ...
        seller = User.objects.filter(pk=r.seller_id).first()
...
def approve_list_invoice_com(request):
    ...
    try:
        obj = []
        # salesorder
        billpa = register_payment.objects.select_related('register').filter(register__pay_type=2,register__status='Y',register__is_event='Y').order_by("-crt_date")[:500]
...
def approve_list_invoice_inv(request):
    ...
    for rs in list(listMenuPermission):
        ...
        objMenu.append(r)
    today = date.today()    
    current_month = int(request.POST.get('monthss', today.month))
    current_year = int(request.POST.get('qyear', today.year))
    ...
    #     st = [0,1]
    #     # salesorder
    #     billpa = register_payment.objects.select_related('register').filter(register__close_the_sale__in=st,register__pay_type=2,register__status='Y').order_by("-crt_date")[:130]
...
@login_required(login_url='/login')
def approve_list_payment_accept_invoice_iv(request):
    user_id = request.user.id
    try:
        m = user_group.objects.get(user=user_id)
    except user_group.DoesNotExist:
    ...
    return redirect("/approve/invoice/acco")   
