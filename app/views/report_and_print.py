...
@login_required(login_url='/login')
def register_report_quotation_by_sale(request):
    user_id = request.user.id
    try:
        m = user_group.objects.get(user=user_id)
    except user_group.DoesNotExist:
    ...
    context = {'title': defaultTitle,  'province_list': province_list,
               'list_user': list_user, 'course_list': course_list, 'daterange': daterange,'listMenuPermission': objMenu}
    return render(request, 'report/register_report_quotation_user.html', context)
...
@login_required(login_url='/login')
def register_excel_quotation_byuser(request):
    user_id = request.user.id
    try:
        m = user_group.objects.get(user=user_id)
    except user_group.DoesNotExist:
    ...
    context = {'title': defaultTitle, 'data': obj,
               'param': param, 'total_sum': total_sum}
    return render(request, 'print/register_excel_quotation_user.html', context)
...
@login_required(login_url='/login')
def register_report_summary_creadit_all_user(request):
    user_id = request.user.id
    try:
        u = user_detail.objects.get(user_id=user_id)
        cm_id = u.cm
    ...
    context = {'title': defaultTitle, 'data': obj,'listMenuPermission': objMenu,'thai_months': THAI_MONTH_NAMES,'current_month':day_current_m}
    return render(request, 'report/billing_cycle_result_summary_withdraw_credit.html', context) 
...
def register_excel_bill(request):
    ...
    for r in content:
        ...
        course_n = course.objects.filter(course_id=r.register.course_id).first()
        
        res = {'main': r, 'customer_list': customer_list,'course':course_n,
               'course_list': course_list, 'payment_list': payment_list}
        obj.append(res)
    # print(total_sum)
    param = {'total_data': len(content), 'range_param': range_param, 'close_the_sale_param': close_the_sale_param,
...
def register_report_summary_user_withdraw_com(request):
    ...
    qs = com_income_setting.objects.filter(status='S',active=0,crt_date__year=m_y,crt_date__month=m_n).values('user_id','user__username','user__first_name','user__last_name').annotate(
    total_before_tax=Coalesce(Sum('tis_com_before_tax'), 0.0),
    total_after_tax=Coalesce(Sum('tis_com_after_tax'), 0.0),
    tax=ExpressionWrapper(
        Coalesce(Sum('tis_com_before_tax'), 0.0) - Coalesce(Sum('tis_com_after_tax'), 0.0),
    ...
    # result_dict = {}
    # customer_order_counts = com_income_setting.objects.filter(status='S',crt_date__year=m_y,crt_date__month=m_n).values('user_id').distinct()
    ...
    #     name = getdatauser.first_name + ' ' + getdatauser.last_name
        ...
        # first = com_income_setting.objects.filter(status='S',active=0)

        # first = com_income_setting.objects.filter(status='S',active=0,crt_date__year=m_y,crt_date__month=m_n,user_id=item_id)
    ...
    # result_list = list(result_dict.values())
    # context = {'title': defaultTitle, 'data': obj,'result_dict':result_list,'tis_group_f':tis_group_f,'tis_group_l':tis_group_l,'old_m':month_fomat(m_l),'current_m':month_fomat(m_n),'year_current':year_current,'m':m_n,'totalp_f':totalp_f,'yearold':yearxx}
    context = {'title': defaultTitle, 'data': obj,'mon':month_fomat(m_n),'year':m_y,'m':m_n,'qs':qs}
